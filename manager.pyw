...
def submit_data(param, pc, oem, duts, notes, window):
    config = ConfigParser()  
...
def button_edit_handler(param):
        ...
        submit_button = tk.Button(editor, text="SUBMIT", command=lambda: submit_data(param, pc_entry.get(), oem_entry.get(), duts_entry.get(), notes_entry.get(), editor))
